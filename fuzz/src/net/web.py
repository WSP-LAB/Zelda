...
class Request:
    ...
    def __hash__(self):
        ...
        if len(path.split(".php")) > 1  :
            path = path.split(".php")[0] + ".php"

        if self._cached_hash is None:
            get_kv = tuple([tuple([param[0], ""]) for param in self._get_params])
            if isinstance(self._post_params, list):
                post_kv = tuple([tuple([param[0], ""]) for param in self._post_params])
            else:
                post_kv = self._enctype + str(len(self._post_params))
            file_kv = tuple([tuple([param[0], param[1][0]]) for param in self._file_params])

            spa_kv = "" 
            if self._get_params != [] and self._get_params[-1][1] != None and "#" in self._get_params[-1][1]:
                spa_kv = self._get_params[-1][1].split("#")[1]
            
            self._cached_hash = hash((self._method, path, get_kv, post_kv, file_kv, spa_kv))
    ...
    def __eq__(self, other):
        ...
        if len(self._resource_path.split(".php")) > 1  :
            self._resource_path = self._resource_path.split(".php")[0] + ".php"
