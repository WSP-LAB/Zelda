...
# Internal libraries
# from src.language.language import _
from src.net import web
from src.net import lamejs
import http
import asyncio
import json 


from src.http_executor import is_network_idle 
...
class NetworkIdle(object):
    def __init__(self):
        self.complete = False

    def __call__(self, driver):
        return driver.execute_script("return (window.performance.getEntriesByType('resource').filter(function(e) { return e.initiatorType == 'xmlhttprequest' && e.duration == 0 }).length === 0)")
...
class Page:
    ...
    def _iter_raw_links(self):
        ...
        for tag in self.soup.find_all("div", "markdown"):
            ...
            for real_tag in markdown_soup.find_all("a", href=True):
                yield real_tag["href"].split("#")[0].strip()
...
class Crawler:
    ...
    def __init__(
            self, base_url: str, timeout: float = 3.0, secure: bool = False, compression: bool = True,
            proxies: dict = None, user_agent: str = None, session: requests.Session = None, browser=None, request_lock=None, request_list=None, mode="headless"):
        self._shared_request = request_list 
        self._shared_lock = request_lock
        self._timeout = timeout
        self._session = session
        self._browser = browser
        #self._session = HTMLSession()
        self.base_url = base_url.replace("http://","").split("/")[1]
        self.base_domain = base_url.replace("http://","").split("/")[1].split(":")[0]
        if user_agent:
    ...
    def is_in_scope(self, resource):
        ...
        if self._scope == Scope.PUNK:
            ...
            if isinstance(resource, web.Request):
                if resource.url[-3:] in [".js","ccs", "svg","png","mp4","jpg"]:
                    return False
                if "setup" in resource.url or "logout" in resource.url or "logoff" in resource.url or "/DVWA/security.php" in  resource.url:
                    return False
                elif self.base_domain in resource.url or "127.0.0.1" in resource.url or "localhost" in resource.url:
                    return True
                else:
                    return False 
            else:
                
                if resource[-3:] in [".js","ccs", "svg","png","mp4","jpg"]:
                    return False
                if "setup" in resource or "logout" in resource or "logoff" in resource or "/DVWA/security.php" in  resource:
                    return False
                elif self.base_domain in resource or "127.0.0.1" in resource or "localhost" in resource:
                    return True
                else:
                    return False
    ...
    def async_get(self, resource: web.Request, follow_redirects: bool = False, headers: dict = None) -> Page:
        """Fetch the given url, returns a Page object on success, None otherwise.
        If None is returned, the error code can be obtained using the error_code property.
        ...
        #print(parse.unquote(resource.url))
        response = self._session.get(
            parse.unquote(resource.url),
            timeout=self._timeout,
            allow_redirects=follow_redirects,
            headers=headers
        )
        ...
        #response1 = self._browser.request.post(form.path,form=self.list_to_dict(post_params),headers=form_headers,ignore_https_errors=True, fail_on_status_code=False,timeout=5000)

        #page
        try: 
            self._browser.implicitly_wait(3)
            self._browser.get(parse.unquote(resource.url))
        ...
        # page.goto(unquote(resource.url), wait_until="networkidle")
        #self._browser.request.get(unquote(resource.url), timeout=5000)
        #print( url.content())
        #print(self._browser.page_source)
        
            response._content = bytes(self._browser.page_source, 'utf-8') 
        except:
            response._content = ""
        #page.close()
    ...
    def intercept (self, req, post_data, post_headers):
   
        if self.reformatFirstRequest :
            self.reformatFirstRequest  = False
            #post_headers['Content-Type'] = 'application/json'
            post_headers = {'referer': 'sdf'}
            post_headers.update(req._headers)
    
            req.continue_(overrides={
                                            'method': 'POST',
                                            'postData': 'username=zeldazelda&password=zeldazelda',
                                            'headers': post_headers
                                            })
        else:
             req.continue_()

    def async_post(self, form: web.Request, follow_redirects: bool = False, headers: dict = None) -> Page:
    ...
    def send(self, resource: web.Request, headers: dict = None, follow_redirects: bool = True) -> Page:
        #print("send: " + resource.url)
        # print(headers)
    ...
    def async_send(self, resource: web.Request, headers: dict = None, follow_redirects: bool = False) -> Page:
        #print("send" + resource.url)
        # print(headers)
        if resource.method == "GET":
            page =  self.async_get(resource, headers=headers, follow_redirects=follow_redirects)
        else:
            page = self.request(resource.method, resource, headers=headers, follow_redirects=follow_redirects)
...
class Explorer:
    def __init__(self, crawler_instance: Crawler):
        ...
        self._hostnames = set()
        self._mode = self._crawler._mode
        #self._mode = ""
    ...
    def explore(
        ...
        while to_explore:
            ...
            try:
                ...
                if self._mode == "headless" and request.method == "GET":
                    page2 =  self._crawler.async_send(request)
                    page_list.append(page2)
                else: 
                    page1 = self._crawler.send(request)
            ...
            for page in page_list:
                ...
                if page.type.startswith(MIME_TEXT_TYPES):
                    ...
                    for extra_url in filter(self._crawler.is_in_scope, page.extra_urls):
                        ...
                        """
                        new_request = self._crawler.compose_request(
                            url=extra_url, 
                            method="GET",
                            encoding='utf-8',
                            enctype=
